...
D = 8
R = holonomy_test(D, seed=0, n=100)
...
def test_path_functionality_certificate():
    ...
    assert R["endpoint_erasure"] < 1e-6  # worldline B really ends where A does
    assert R["dzeta_min"] > 1e-6  # yet every pair differs in ζ
    assert R["dzeta_mean"] > 0.1 * R["zeta_scale"]  # by the order of ζ itself, not a rounding sliver


def test_zeta_replica_test():
    """The ζ-reader tells a lived worldline from a fresh copy at the same observable."""
    assert R["auc_zeta"] == 1.0


def test_zeta_accumulates():
